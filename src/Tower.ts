...
export class Tower {
    ...
    getStats(): IProjectileStats & { range: number, cd: number, projCount: number, spread: number } {
        ...
        this.cards.forEach(c => {
            ...
            if(c.type.id === TYPES.FIRE.id) {
                ...
                if (lvl === 1) { s.dmg += 5; s.cd *= 1.15; s.effects.push({ type: 'splash', radius: 40 }); } 
                else if (lvl === 2) { s.dmg += 15; s.cd *= 1.10; s.effects.push({ type: 'splash', radius: 70 }); } 
                else if (lvl >= 3) { s.dmg += 25; s.effects.push({ type: 'splash', radius: 70 }); s.effects.push({ type: 'killExplode', power: 0.5 }); }
            }
            else if(c.type.id === TYPES.ICE.id) {
                s.color = TYPES.ICE.color;
                if (lvl === 1) { s.range -= 20; s.effects.push({ type: 'slow', power: 0.2, dur: 40 }); } 
                else if (lvl === 2) { s.range -= 40; s.effects.push({ type: 'slow', power: 0.35, dur: 60, amp: 0.2 }); } 
                else if (lvl >= 3) { s.range -= 40; s.effects.push({ type: 'slow', power: 0.65, dur: 80, amp: 0.25 }); s.effects.push({ type: 'killFreeze' }); }
            }
            else if(c.type.id === TYPES.SNIPER.id) {
                s.color = TYPES.SNIPER.color; s.speed = 18;
                if (lvl === 1) { s.dmg += 20; s.cd *= 1.5; s.range += 60; s.critChance += 0.25; } 
                else if (lvl === 2) { s.dmg += 40; s.cd *= 1.5; s.range += 100; s.critChance += 0.35; } 
                else if (lvl >= 3) { s.dmg += 70; s.cd *= 1.3; s.range += 150; s.critChance += 0.50; s.pierce += 2; }
            }
            else if(c.type.id === TYPES.MULTISHOT.id) {
                s.color = TYPES.MULTISHOT.color; s.spread = 0.2;
                if (lvl === 1) { s.projCount = 2; s.dmg = Math.floor(s.dmg * 0.55); } 
                else if (lvl === 2) { s.projCount = 2; s.dmg = Math.floor(s.dmg * 0.65); } 
                else if (lvl >= 3) { s.projCount = 3; s.dmg = Math.floor(s.dmg * 0.50); }
            }
        });
    ...
    update(enemies: Enemy[], projectiles: Projectile[], pool: ObjectPool<Projectile>, audio: AudioSystem) {
        ...
        if(target) {
            ...
            if(this.cooldown <= 0) { 
                ...
                // ПРОИГРЫВАЕМ ЗВУК
                // Определяем тип звука по первой карте
                let soundType: any = 'default';
                if (this.cards.length > 0) soundType = this.cards[0].type.id;
                audio.playShoot(soundType);
